...
interface PushToApolloResult {
  attendee: Attendee
  success: boolean
  error?: string
}

interface PushToApolloResultsDialogProps {
  isOpen: boolean
  onClose: () => void
  results: PushToApolloResult[]
  listName: string
}

export function PushToApolloResultsDialog({ 
  isOpen, 
  onClose, 
  results,
  listName 
}: PushToApolloResultsDialogProps) {
        ...
        <div className="w-full max-w-2xl bg-white rounded-2xl shadow-xl flex flex-col h-[80vh] max-h-[600px]">
          ...
          <div className="p-6 border-b border-gray-200">
            ...
            <h3 className="text-lg font-medium leading-6 text-gray-900 text-center mb-2">
              Push to Apollo Complete
            </h3>
            
            <p className="text-sm text-center text-gray-600 mb-4">
              Contacts pushed to list: <span className="font-medium">{listName}</span>
            </p>
            ...
            <dl className="grid grid-cols-3 gap-x-8 text-center">
              ...
              <div className="mx-auto flex max-w-xs flex-col gap-y-2">
                <dt className="text-base text-gray-600">Successfully Added</dt>
                <dd className="order-first text-3xl font-semibold tracking-tight text-green-600">
                  {successCount}
                </dd>
              </div>
              <div className="mx-auto flex max-w-xs flex-col gap-y-2">
                <dt className="text-base text-gray-600">Failed to Add</dt>
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-xs text-gray-500">
                          {result.success ? 'Added to Apollo' : 'Failed'}
                        </span>
                      </div>
