...
export default function HealthSystemsPage() {
  return (
    <SelectionProvider>
      <HealthSystemsPageContent />
    </SelectionProvider>
  )
}

function HealthSystemsPageContent() {
  const router = useRouter()
  const { selectedItems } = useSelection()
  const { healthSystems, isLoading, error, totalCount, fetchData, hasMore, currentPage, setCurrentPage: _setCurrentPage } = useDataFetching()
  ...
  // Add handler for loading more health systems
  const handleLoadMore = useCallback(() => {
    ...
    console.log('Loading more health systems, current page:', currentPage, 'next page:', nextPage, 'timestamp:', new Date().toISOString());
    ...
    <div className="flex h-screen">
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 relative z-40">
            <div className="flex flex-col gap-2 w-full lg:max-w-xl">
              ...
              <div className="text-sm text-gray-500 ml-1">
                <span>{totalCount.healthSystems} total health systems</span>
                {(searchTerm || activeFilters.length > 0) && (
                  <span> • {healthSystems.length} results showing</span>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-wrap lg:flex-nowrap items-center gap-2 whitespace-nowrap min-w-fit lg:w-auto lg:ml-auto relative z-40">
              ...
              <PropertiesMenu
                visibleColumns={visibleColumns['health-systems']}
          ...
          {/* Main content area */}
          <div className="mt-6">
            {isLoading && healthSystems.length === 0 ? (
            ...
            ) : healthSystems.length === 0 && !isLoading && !searchTerm && activeFilters.length === 0 ? (
              <div className="flex flex-col items-center justify-center h-64">
                <p className="text-gray-500 mb-4">No health systems available. Would you like to add one?</p>
                <AddEntityButton 
                  entityType="health-systems"
                  onEntityAdded={handleEntityAdded}
                />
              ...
              <div className="bg-white rounded-lg shadow-sm overflow-hidden border border-gray-100">
                <DataTable<HealthSystem>
                  ...
                  onRowClick={(healthSystem) => router.push(`/health-systems/${healthSystem.id}`)}
                />
              </div>
            ) : (
              <div className={`grid gap-4 relative z-0 ${
                ...
                {healthSystems.map((healthSystem, index) => (
                  <ItemCard
                    key={`${healthSystem.id}-${index}`}
                    title={healthSystem.name}
                    subtitle={healthSystem.city && healthSystem.state ? `${healthSystem.city}, ${healthSystem.state}` : ''}
                    icon={<Icon icon={BuildingOfficeIcon} size="sm" className="text-gray-400" />}
                    onClick={() => router.push(`/health-systems/${healthSystem.id}`)}
                    item={healthSystem}
      ...
      <ActionBar 
        onEnrichmentComplete={() => fetchData({ entityType: 'health-systems' })}
        onDefinitiveEnrichmentComplete={() => fetchData({ entityType: 'health-systems' })}
        onAIEnrichmentComplete={() => fetchData({ entityType: 'health-systems' })}
        onDelete={() => fetchData({ entityType: 'health-systems' })}
        conferenceName=""
        activeListId={activeListId}
        onListDelete={() => {
